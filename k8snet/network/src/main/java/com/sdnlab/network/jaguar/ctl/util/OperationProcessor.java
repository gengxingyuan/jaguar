...
public final class OperationProcessor implements AutoCloseable, Runnable {
    ...
    private static final String VXLAN_MANAGER = "VxlanInterfaceManager";

    private final BlockingQueue<InterfaceOperation> queue = new LinkedBlockingQueue<>(OPERATION_QUEUE_DEPTH);
    private final Thread thread;
    private TransactionChainManager transactionChainManager;
    private volatile boolean finishing = false;

    public OperationProcessor(final DataBroker dataBroker) {
        transactionChainManager = new TransactionChainManager(dataBroker, VXLAN_MANAGER);
        transactionChainManager.activateTransactionManager();
        transactionChainManager.initialSubmitWriteTransaction();

        thread = new Thread(this);
        thread.setDaemon(true);
        thread.setName("VxlanInterfaceManager");
    }

    public void enqueueOperation(final InterfaceOperation task) {
        try {
            queue.put(task);
    ...
    public void run() {
        while (!finishing) {
            try {
                InterfaceOperation op = queue.take();
